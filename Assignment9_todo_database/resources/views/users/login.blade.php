...
<form action="/login" method="POST" class="bg-white p-8 rounded-2xl shadow-lg w-80 space-y-6">
    ...
    {!! Form::open(['route' => 'users.loginUser', 'method' => 'post', 'class' => 'space-y-2 h-auto w-full']) !!}

            <div>
                {!! Form::label('userid', 'User ID', ['class' => 'block']) !!}
                {!! Form::text('userid', null, ['required', 'class' => 'w-full p-2 border border-black rounded-lg']) !!}
            </div>

            <div>
                {!! Form::label('password', 'Password', ['class' => 'block']) !!}
                {!! Form::password('password', null, ['required', 'class' => 'w-full  border border-black rounded-lg']) !!}
            </div>
